...
public final class LinuxTrayIcon extends AbstractTrayIcon {
    ...
    private AbstractTrayIcon createDbusDelegate() {
        try {
            final Class<?>         cls  = Class.forName("eu.hansolo.trayfx.impl.linux.LinuxDbusImpl");
            final AbstractTrayIcon impl = (AbstractTrayIcon) cls.getDeclaredConstructor(AbstractTrayIcon.class).newInstance(this);
            return impl;
        } catch (final Exception e) {
            return null; // D-Bus unavailable -> AWT fallback will be used
        }
    }
    ...
    private static final class AwtTrayIconDelegate extends AbstractTrayIcon {
        ...
        @Override protected void nativeInstall() {
            if (!SystemTray.isSupported()) { throw new UnsupportedOperationException("SystemTray not supported and D-Bus unavailable."); }
            offThread(() -> {
                try {
        ...
        private void showFreshMenu(final MouseEvent e) {
            final TrayMenu          trayMenu = getMenu(); if (trayMenu == null || trayMenu.isEmpty()) { return; }
            final java.awt.TrayIcon trayIcon = awtTrayIcon; if (trayIcon == null) { return; }
            final PopupMenu popup = new PopupMenu();
        ...
        private BufferedImage toBufferedImage(final Image fxImage) {
            ...
            g2d1.drawImage(argb, 0, 0, size, size, null);
            g2d1.dispose();
            return scaled;
        }
