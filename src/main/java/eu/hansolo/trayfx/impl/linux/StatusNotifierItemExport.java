 ...
 */
public final class StatusNotifierItemExport implements StatusNotifierItemInterface, Properties {
    ...
    StatusNotifierItemExport(final DBusConnection connection, final Consumer<Void> onActivate, final Consumer<Void> onContextMenu) {
    ...
    void setIcon(final Image fxImage) {
        this.icon        = fxImage;
        this.iconPixmaps = fxImage != null ? toArgb32Pixmaps(fxImage) : Collections.emptyList();
        try { emitSignal(new StatusNotifierItemInterface.NewIcon(OBJECT_PATH)); } catch (final Exception ignored) {}
    }

    void setTitle(final String title) {
        this.title = title != null ? title : "";
        try { emitSignal(new StatusNotifierItemInterface.NewTitle(OBJECT_PATH)); } catch (final Exception ignored) {}
    }

    void setToolTip(final String tip) {
        this.toolTip = tip != null ? tip : "";
        try { emitSignal(new StatusNotifierItemInterface.NewToolTip(OBJECT_PATH)); } catch (final Exception ignored) {}
    }
    ...
    @Override public void Activate(final int x, final int y) { if (onActivate != null) { onActivate.accept(null); } }

    @Override public void ContextMenu(final int x, final int y) { if (onContextMenu != null) { onContextMenu.accept(null); } }
    ...
    @Override public <A> A Get(final String iface, final String name) {
        @SuppressWarnings("unchecked")
        A val = (A) getProperties().get(name);
        return val;
    }

    @Override public <A> void Set(final String iface, final String name, final A value) {}

    @Override public Map<String, Variant<?>> GetAll(final String iface) {
    ...
    static List<Object[]> toArgb32Pixmaps(final Image fxImage) {
        ...
        pixmaps.add(new Object[]{ width, height, bytes });
        return pixmaps;
    }
